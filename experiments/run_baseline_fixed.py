...
def run_fixed_baseline(seed, generations=10000):
    random.seed(seed)
    np.random.seed(seed)
    ...
    print(f"Starting Fixed Baseline [Seed {seed}, {generations} gens]")
    
    # Initialize static substrate
    ...
    # The prompt requests individual logs to results/baselines/seed_<seed>/
    # I'll just write to results/baselines/fixed_seed_<seed>.csv for simplicity
    log_path = f'results/baselines/fixed_seed_{seed}.csv'
    ...
    for gen in range(1, generations + 1):
        ...
        if gen % log_interval == 0:
            ...
            print(f"Fixed [Seed {seed}] Gen {gen:05d} | Nodes: {avg_nodes:.2f} | LZ: {avg_lz:.3f}")
...
if __name__ == "__main__":
    ...
    run_fixed_baseline(args.seed, args.generations)
